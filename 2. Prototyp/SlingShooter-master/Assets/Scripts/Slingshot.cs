...
public class Slingshot : MonoBehaviour {
	...
	public GameObject prefabGearwheel;
	public float velocityMult = 4f;
	
	// Fields set dynamically
	private GameObject launchPoint;
	private Vector3 launchPos;
	private GameObject Gearwheel;
	...
	void OnMouseDown(){
		...
		Gearwheel = Instantiate(prefabGearwheel) as GameObject;

		// Start it at launch position
		Gearwheel.transform.position = launchPos;

		// Set it to kinematic for now
		Gearwheel.GetComponent<Rigidbody>().isKinematic = true;
	}
	...
	void Update() {
		...
		Gearwheel.transform.position = launchPos + mouseDelta;
		
		if(Input.GetMouseButtonUp(0)) {
			// The mouse has been released
			aimingMode = false;
			// Fire off the projectile with given velocity
			Gearwheel.GetComponent<Rigidbody>().isKinematic = false;
			Gearwheel.GetComponent<Rigidbody>().velocity = -mouseDelta * velocityMult;

			// Set the Followcam's target to our projectile
			FollowCam.S.poi = Gearwheel;
